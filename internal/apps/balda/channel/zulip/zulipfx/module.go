	...
	fx.Provide(
		fx.Annotate(
			func(enabled bool) commandcmd.Advertisement {
				return commandcmd.Advertisement{Transport: zulip.ChannelType, Enabled: enabled, Names: []string{"locator", "reset", "usage", "auto", "cancel", "goalkeeper", "topic", "close", "start", "user"}}
			},
