...
func (h *CommandHandler) formatSwarmStatus(ctx context.Context) (string, error) {
	...
	if h.commandBus != nil {
		...
		fmt.Fprintf(&out, "%d", status.CommandsPublishedTotal)
		out.WriteString("\n- commands_running_total: ")
		fmt.Fprintf(&out, "%d", status.CommandsRunningTotal)
		out.WriteString("\n- commands_acked_total: ")
		fmt.Fprintf(&out, "%d", status.CommandsAckedTotal)
		out.WriteString("\n- commands_retrying_total: ")
		fmt.Fprintf(&out, "%d", status.CommandsRetryingTotal)
		out.WriteString("\n- commands_deadlettered_total: ")
		fmt.Fprintf(&out, "%d", status.CommandsDeadletteredTotal)
		out.WriteString("\n- command_duration_seconds: ")
		fmt.Fprintf(&out, "%.3f", status.CommandDurationSeconds)
		out.WriteString("\n- actor_duration_seconds: ")
		fmt.Fprintf(&out, "%.3f", status.ActorDurationSeconds)
