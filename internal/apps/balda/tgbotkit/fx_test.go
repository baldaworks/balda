...
func TestNewUpdateSource_WebhookModeRequiresURL(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "balda.telegram.webhook.enabled=true") {
		t.Fatalf("NewUpdateSource() error = %q, want contains balda.telegram.webhook.enabled=true", err)
	}
}
