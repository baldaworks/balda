...
func (h *zulipInboundHandler) HandleCommand(ctx context.Context, cmd zulip.InboundCommand) error {
	...
	if h.commandIngress != nil && (cmd.Command == commandReset || cmd.Command == commandLocator || cmd.Command == commandUsage || cmd.Command == commandAuto || cmd.Command == commandCancel || cmd.Command == commandGoal || cmd.Command == commandTopic || cmd.Command == commandClose || cmd.Command == commandStart || cmd.Command == commandUser || cmd.Command == commandSkill) {
