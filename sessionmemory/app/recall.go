...
// disposable lexical projection.  Projection text is never returned without
// a canonical read for the same exact scope.
...
// NewRecallService constructs bounded canonical recall.  A nil projection is
// valid and selects the bounded canonical-tail fallback.
...
// Search executes one exact-scope recall request.
func (s *RecallService) Search(ctx context.Context, request sessionmemory.RecallRequest) (sessionmemory.RecallResponse, error) {
	...
	if s.projection == nil {
		records, err := s.canonical.SearchRecallTail(ctx, normalized, recallTailLimit(normalized.Limit))
		if err != nil {
			return sessionmemory.RecallResponse{}, err
		}
...
func (s *RecallService) responseFromProjection(request sessionmemory.RecallRequest, records []sessionmemory.RecallRecord, hits map[string]sessionmemory.RecallProjectionHit, currentSeq uint64, now time.Time) (sessionmemory.RecallResponse, error) {
	...
	for _, record := range records {
		...
		results = append(results, recallReference(record, sessionmemory.RankRecall(hit.Score, record.CreatedAt, now)))
	...
	return recallResponse(request.Scope, currentSeq, results), nil
}
...
func recallReference(record sessionmemory.RecallRecord, score sessionmemory.RecallScore) sessionmemory.RecallReference {
	...
	var category *sessionmemory.AtomCategory
	if record.Category != nil {
		copyOfCategory := *record.Category
		category = &copyOfCategory
	}
...
func emptyRecallResponse(scope sessionmemory.Scope, sequence uint64) sessionmemory.RecallResponse {
	return recallResponse(scope, sequence, []sessionmemory.RecallReference{})
}

func recallResponse(scope sessionmemory.Scope, sequence uint64, results []sessionmemory.RecallReference) sessionmemory.RecallResponse {
	return sessionmemory.RecallResponse{SchemaVersion: sessionmemory.RecallSchemaVersionV1, Trust: sessionmemory.ReferenceTrustUntrusted, Scope: scope, ScopeChangeSeq: sequence, Results: results}
}

var _ RecallReader = (*RecallService)(nil)
