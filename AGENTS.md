...
- `/reset`: owner/collaborator; cancels current session work, clears current session history, and immediately starts a fresh runtime session without closing the chat/topic.
- `/skill <skill> [prompt...]` or `/skill <plugin>:<skill> [prompt...]`: owner/collaborator; runs one skill from the current session snapshot as an ordinary turn. Slack uses `/balda skill ...`.
...
- `cmd/backoffice` is the Backoffice executable and composition edge.
- `internal/apps/backoffice` owns Backoffice application behavior, including the Web UI.
- Before changing either area, you MUST read and follow `docs/reference/backoffice.md`.
