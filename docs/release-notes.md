...
- Reprints the owner bootstrap command and deeplink on `balda start` while the bot is still unclaimed, then stops exposing them again after the first successful owner auth.
...
- Defaults `balda.sessions.persistence` to `sqlite`, keeping ADK conversation history across restarts until the session is explicitly closed.
- Surfaces ACP plan updates in Telegram progress and keeps them configurable via `balda.telegram.plan_updates`.
