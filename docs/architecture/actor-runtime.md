...
- Provider runtime: `balda.provider` selects the single app-scoped provider runtime used by all Balda sessions and `/goalkeeper` worker-validator runs; goal runs get isolated worker/validator ADK sessions and workspace state but reuse that same provider runtime/client ownership.
...
- Telegram/Zulip/Slack chat/webhook/scheduler ingress lives in `internal/apps/balda/handlers`; handlers normalize inbound provider data, enforce ingress preconditions, and publish actor commands without owning turn execution. Concrete provider-runtime bindings live in `internal/apps/balda/handlersfx`.
